...
def sgd(cost, params, lr=0.05, decay=1e-6):
...
pylab.savefig('sgd_train_error.png')
...
pylab.savefig('sgd_test_accuracy.png')
...
for sample in range(2):
    ...
    pylab.savefig('sgd'+str(sample)+'_question1_input.png')

    # images for 25 iterations
    for i in range(25):
        convolved1, pooled1,convolved2,pooled2 = test(teX[ind:ind+1,:])
        #  At Convolution 1
        pylab.figure(1)
        pylab.gray()
        pylab.subplot(5, 5, i+1); pylab.axis('off'); pylab.imshow(convolved1[0,0,:].reshape(20,20))
        pylab.savefig('sgd'+str(sample)+'_question1_convolution1.png')

        # At pooling 1
        pylab.figure(2)
        pylab.gray()
        pylab.subplot(5, 5, i+1); pylab.axis('off'); pylab.imshow(pooled1[0,0,:].reshape(10,10))
        pylab.savefig('sgd'+str(sample)+'_question1_pooling1.png')

        # At convolution 2
        pylab.figure(3)
        pylab.gray()
        pylab.subplot(5, 5, i+1); pylab.axis('off'); pylab.imshow(convolved2[0,0,:].reshape(6,6))
        pylab.savefig('sgd'+str(sample)+'_question1_convolution2.png')

        # At pooling 2
        pylab.figure(4)
        pylab.gray()
        pylab.subplot(5, 5, i+1); pylab.axis('off'); pylab.imshow(pooled2[0,0,:].reshape(3,3))
        pylab.savefig('sgd'+str(sample)+'_question1_pooling2.png')
